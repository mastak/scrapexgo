...
type Options struct {
	ConfigPath  flags.Filename `short:"c" long:"config_path" default:"../config/scrapex.yml" env:"CONFIG_PATH" description:"Path To configure"`
	ScrapeTest  bool           `short:"t" long:"test" description:"To test for crawling via a scraper only\n(Without an emitter module)"`
	ShowVersion bool           `short:"v" long:"version" description:"Show scrapex version and git revision id"`
}
...
func setOptions() (model.Config, error) {
	...
	parser.LongDescription = `Options for scrapex`
	...
	if options.ShowVersion {
		fmt.Printf("scrapex %s (git commit: %s)\n", version, commit)
		os.Exit(0)
	}
